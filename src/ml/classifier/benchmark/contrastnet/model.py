...
class Contrastive_Loss(nn.Module):
    ...
    def forward(self, batch_label, *x):
        ...
        if len(x)==1:
            mask_i = torch.from_numpy(np.ones((len_, len_), dtype=np.float32)).to(batch_labels.device)
        else:
            mask_i = 1. - torch.from_numpy(np.identity(len_, dtype=np.float32)).to(batch_labels.device) # sum over items in the numerator
...
if __name__ == '__main__':
    ...
    # n way k fewshot classification means that we have n classes and k samples per class
    # xs is the support set and xq is the query set
    # task_xs is the unsupervised support set
    # x_augment is the augmented support set
    ...
    output_path = f'{file_dir}/runs/{datetime.now()}'
    dataset=f"{file_dir}/data/BANKING77"
    tuned="tdopierre/ProtAugment-LM-BANKING77"
    augmentation=f"{file_dir}/data/BANKING77/paraphrases/DBS-unigram-flat-1.0/paraphrases.json"
    cv = "01"
    ...
    data_path = f"{dataset}/full.json"
    train_path = f"{dataset}/few_shot/{cv}/train_aug.json"

    # Create variables from the shell script arguments
    train_labels_path = f"{dataset}/few_shot/{cv}/labels.train.txt"
    valid_labels_path = f"{dataset}/few_shot/{cv}/labels.valid.txt"
    test_labels_path = f"{dataset}/few_shot/{cv}/labels.test.txt"
    unlabeled_path = f"{dataset}/raw.txt"
    log_path = f"{logdir}/{cv}.out"
    ...
    model_name_or_path = tuned
    n_unlabeled = 10
    n_task = 10
    augmentation_data_path = augmentation

    metric = "euclidean"
    supervised_loss_share_power = 1
    ...
    for k in train_dataset.data.keys():
        print(k, len(train_dataset.data[k]))
    ...
    for step in range(max_iter):
        ...
        print(f'step: {step} / {max_iter} step(s)')
        ...
        # why again?
        #model.train_step(optimizer=optimizer, episode=episode, supervised_loss_share=supervised_loss_share, task_loss_share=task_loss_share)
        set_results = model.test_step(
            dataset=valid_dataset,
            n_episodes=n_test_episodes
        )
        
        logger.info(f"valid | " + " | ".join([f"{key}:{np.mean(value):.4f}" for key, value in set_results.items()]))
